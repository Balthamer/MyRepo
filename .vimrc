...
autocmd FileType tex inoremap ;s \documentclass[10pt,letterpaper]{article}<Enter><Enter>\usepackage{parskip}<Enter>\usepackage[margin=1in]{geometry}<Enter>\usepackage{titlesec}<Enter>\titleformat{section}{\large\bfseries}{}{0em}{}<Enter><Enter>\title{<++>}<Enter>\author{Michael D. Allred}<Enter><Enter>\begin{document}<Enter><Enter>\maketitle<Enter><Enter>\section{<++>}<Enter><Enter><++><Enter><Enter>\end{document}<Esc>/<++><Enter>"_c4l
